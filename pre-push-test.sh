...
if [[ $res -ne 0 ]]; then
    ...
    git branch -a | grep -q "origin/$curBranch$"
    res=$?
    if [[ 0 -ne $res ]]; then
        curBranch="master"
    else
		...
		if [[ 0 -eq $res ]]; then
			...
			if [[ $res -ne 0 ]]; then
				...
				if [[ $* == *--dry-run* ]]; then
					...
					res=$?
					# bugfix: del line: let cmd = process.platform=='win32' ? 'npm.cmd' : 'npm';
					#  Cannot find module 'E:\gitdata\github\eslint-rocket\scripts\node_modules\npm\bin\npm-cli.js'
					if [[ 0 -ne $res ]]; then
						exit $res
					fi
				fi
